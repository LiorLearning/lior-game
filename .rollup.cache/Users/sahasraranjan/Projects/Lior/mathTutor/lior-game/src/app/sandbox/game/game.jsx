...
function Bar({ parts, selectedParts, onCut, onJoin, onSelect, maxParts, compare = false, disabled = false, label, expectedFraction, }) {
    ...
    const handleSelect = (part) => {
        if (!disabled) {
            onSelect === null || onSelect === void 0 ? void 0 : onSelect(part);
        }
    };
    ...
    return (<div className="relative">
      <div className="flex items-center gap-6">
          ...
          <div className="w-full perspective-1000">
            <div className={`relative h-32 rounded-lg shadow-xl transform-style-3d rotate-x-10 
              ...
              <div className="absolute inset-0 flex gap-1 p-1">
                ...
                {Array.from({ length: parts }).map((_, index) => (<Button key={index} onClick={!compare ? () => handleSelect(index) : undefined} id={`select-chocolate-piece-${label}-${index}`} className={`flex-1 relative bg-gradient-to-b from-[#8a5a42] via-[#734939] to-[#5c3624] 
                ...
                : selectedParts.includes(index)
                    ...
                    : ''}`}>
                    {compare && !selectedParts.includes(index) && (<div className="absolute inset-0 bg-gray-500 opacity-70 z-20 pointer-events-none"></div>)}
                    ...
                    <div className="absolute inset-0 flex flex-col justify-around py-2">
                      {[0, 1].map((groove) => (<div key={groove} className="relative w-full h-2">
        ...
        <div className="w-40 flex flex-col gap-4">
            <Button onClick={handleBreak} id={`split-button-${label}`} className={`flex-1 h-14 rounded-xl shadow-lg transition-all duration-300
            ...
            : parts >= maxParts
                ? 'cursor-not-allowed opacity-50'
                : 'hover:shadow-xl hover:scale-105 active:scale-95'}`} disabled={compare || parts >= maxParts}>
              <span className={`transform transition-transform duration-300 ${isAnimating ? 'animate-split' : ''}`}>
                Split 
              </span> 🍫
            </Button>
            {parts > 1 && (<Button onClick={!compare && parts > 1 ? handleJoin : undefined} id={`join-button-${label}`} className={`flex-1 h-14 rounded-xl shadow-lg transition-all duration-300
                flex items-center justify-center p-2
                ...
                : parts <= 1
                    ? 'cursor-not-allowed opacity-50'
                    : (!disabled && parts > expectedFraction.denom ? 'animate-bounce' : 'hover:shadow-xl hover:scale-105 active:scale-95 cursor-pointer')}`} disabled={compare || parts <= 1}>
                <p>Join</p>🍯
              </Button>)}
        </div>
      </div>
...
const maxParts = 12;
const FractionsGame = ({ sendAdminMessage }) => {
    const { gameState, setGameState } = useGameState();
    const checkFraction = (bar, targetFraction) => {
        return bar.parts === targetFraction.denom && bar.selectedParts.length === targetFraction.num;
    };
    const handleCut = (barNumber) => {
        setGameState(prev => (Object.assign(Object.assign({}, prev), { [barNumber === 1 ? 'bar1' : 'bar2']: Object.assign(Object.assign({}, prev[barNumber === 1 ? 'bar1' : 'bar2']), { parts: prev[barNumber === 1 ? 'bar1' : 'bar2'].parts + 1 }) })));
    };
    const handleJoin = (barNumber) => {
        setGameState(prev => (Object.assign(Object.assign({}, prev), { [barNumber === 1 ? 'bar1' : 'bar2']: Object.assign(Object.assign({}, prev[barNumber === 1 ? 'bar1' : 'bar2']), { parts: Math.max(prev[barNumber === 1 ? 'bar1' : 'bar2'].parts - 1, 1) }) })));
    };
    useEffect(() => {
        const isFirstFractionCorrect = checkFraction(gameState.bar1, gameState.fraction1);
        setGameState(prev => (Object.assign(Object.assign({}, prev), { isFirstFractionCorrect })));
    ...
    useEffect(() => {
        ...
        setGameState(prev => (Object.assign(Object.assign({}, prev), { isSecondFractionCorrect })));
    ...
    const handleSelect = (barNumber, part) => {
        setGameState(prev => (Object.assign(Object.assign({}, prev), { [barNumber === 1 ? 'bar1' : 'bar2']: Object.assign(Object.assign({}, prev[barNumber === 1 ? 'bar1' : 'bar2']), { selectedParts: prev[barNumber === 1 ? 'bar1' : 'bar2'].selectedParts.includes(part)
    ...
    const handleAnswer = (answer) => {
        ...
        setGameState(prev => (Object.assign(Object.assign({}, prev), { userAnswer: answer, showAnswer: true })));
    ...
    const handleCompare = () => {
        ...
        setGameState(prev => (Object.assign(Object.assign({}, prev), { compareMode: true })));
    };
    const startGame = () => {
        setGameState(prev => (Object.assign(Object.assign({}, prev), { gameStarted: true })));
    ...
    return (<Card className="w-full max-h-4xl max-w-6xl p-16 bg-gradient-to-br from-[#faf4eb] to-[#f5e6d3] shadow-2xl rounded-2xl mx-4 overflow-auto">
      <div className="space-y-8">
        ...
        {/* Visualise Button */}
        {!gameState.gameStarted && (<div className="flex justify-center">
            <Button onClick={startGame} id="visualise-button" className="p-6 font-bold text-lg bg-gradient-to-r from-[#8B4513] to-[#A0522D] text-white hover:shadow-xl transition-all duration-300 transform hover:scale-110 active:scale-95 animate-bounce">
              Visualise
            </Button>
          </div>)}

        {/* Chocolate Bars Container */}
        {gameState.gameStarted && (<div className="space-y-12 relative">
            <div className={`transition-all duration-500 ${gameState.showAnswer ? 'opacity-90 filter contrast-75' : ''}`}>
              <div className="flex items-center mb-4">
                <div className="flex-1">
                  ...
                  {gameState.isFirstFractionCorrect && (<span className="ml-2 text-green-600 animate-bounce">✓</span>)}
                </div>
              </div>
              <Bar parts={gameState.bar1.parts} selectedParts={gameState.bar1.selectedParts} onCut={() => handleCut(1)} onJoin={() => handleJoin(1)} onSelect={(part) => handleSelect(1, part)} maxParts={maxParts} compare={gameState.compareMode} disabled={gameState.isFirstFractionCorrect} label="first" expectedFraction={gameState.fraction1}/>
            </div>
            
            {gameState.isFirstFractionCorrect && (<div className={`transition-all duration-500 ${gameState.showAnswer ? 'opacity-90 filter contrast-75' : ''}`}>
                <div className="flex items-center mb-4">
                  <div className="flex-1">
                  <span className="text-lg font-semibold text-[#5d4037]">Second Bar: Make {gameState.fraction2.num}/{gameState.fraction2.denom}</span>
                  {gameState.isSecondFractionCorrect && (<span className="ml-2 text-green-600 animate-bounce">✓</span>)}
                </div>
              </div>
              <Bar parts={gameState.bar2.parts} selectedParts={gameState.bar2.selectedParts} onCut={() => handleCut(2)} onJoin={() => handleJoin(2)} onSelect={(part) => handleSelect(2, part)} maxParts={maxParts} compare={gameState.compareMode} disabled={gameState.isSecondFractionCorrect} label="second" expectedFraction={gameState.fraction2}/>
              </div>)}
          </div>)}

        {/* Comparison Buttons */}
        {!gameState.showAnswer && !gameState.compareMode && gameState.gameStarted && (<div className="flex flex-col items-center gap-4 mt-12">
            <div className="flex justify-center gap-6">
              <Button onClick={handleCompare} id="compare-button" className={`px-8 py-6 text-lg font-bold rounded-xl shadow-lg
                ...
                : 'bg-gray-200 text-gray-400 cursor-not-allowed'}`} disabled={!gameState.isFirstFractionCorrect || !gameState.isSecondFractionCorrect}>
                Compare Bars
              </Button>
            </div>
          </div>)}

        {gameState.compareMode && !gameState.showAnswer && (<div className="flex flex-col items-center gap-4 mt-12">
            <div className="flex justify-center gap-6">
              <Button onClick={() => handleAnswer(`${gameState.fraction1.num}/${gameState.fraction1.denom}`)} id="fraction1-button" className={`px-8 py-6 text-lg font-bold rounded-xl shadow-lg
                  transition-all duration-300 transform hover:scale-105 active:scale-95
                  bg-gradient-to-r from-[#8B4513] to-[#A0522D] text-white hover:shadow-xl`}>
                {gameState.fraction1.num}/{gameState.fraction1.denom} is bigger
              </Button>
              <Button onClick={() => handleAnswer(`${gameState.fraction2.num}/${gameState.fraction2.denom}`)} id="fraction2-button" className={`px-8 py-6 text-lg font-bold rounded-xl shadow-lg
                  transition-all duration-300 transform hover:scale-105 active:scale-95
                  ${gameState.isFirstFractionCorrect && gameState.isSecondFractionCorrect
                ? 'bg-gradient-to-r from-[#8B4513] to-[#A0522D] text-white hover:shadow-xl'
                : 'bg-gray-200 text-gray-400 cursor-not-allowed'}`} disabled={!gameState.isFirstFractionCorrect || !gameState.isSecondFractionCorrect}>
