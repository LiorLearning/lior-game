...
const Chat = ({ desc, gameState, componentRef }) => {
    ...
    }, [messageContext === null || messageContext === void 0 ? void 0 : messageContext.messages]);
    const renderMessage = (message) => {
        ...
        if (isAgent) {
          ...
          <div className="max-w-[70%] p-3 rounded-2xl bg-secondary text-secondary-foreground shadow-sm">
            ...
            <Button size="sm" variant="outline" className="rounded-xl px-2 py-1" onClick={() => toggleAudio(message)}>
    ...
    return (<div className="flex flex-col h-screen bg-background">
      ...
      <ScrollArea className="flex-1 p-4 overflow-y-auto" ref={scrollRef}>
        <div className="space-y-4">
          {messageContext === null || messageContext === void 0 ? void 0 : messageContext.messages.slice(-MAX_MESSAGES).map((message, index) => (<React.Fragment key={index}>
      ...
      <div className="border-t bg-background p-4">
        <div className="flex items-center gap-2 max-w-4xl mx-auto">
          <Input type="text" value={inputMessage} onChange={(e) => setInputMessage(e.target.value)} placeholder="Type your message..." className="flex-1 rounded-xl border-border focus:border-primary focus:ring-2 focus:ring-primary" onKeyDown={(e) => {
            if (e.key === 'Enter') {
                e.preventDefault();
                onSendTextMessage();
            }
        }}/>
          <Button onClick={onSendTextMessage} size="icon" className="rounded-2xl bg-primary-foreground text-primary hover:bg-primary hover:text-primary-foreground transition-colors">
