...
export const AudioProvider = ({ children, clientId, setIsPlaying }) => {
    ...
    const streamCloseTimeoutRef = useRef(null);
    // Function to unlock AudioContext on user interaction
    const unlockAudioContext = () => {
        if (audioContextRef.current && audioContextRef.current.state === 'suspended') {
            audioContextRef.current.resume().then(() => {
                var _a;
                console.log('AudioContext resumed on user interaction:', (_a = audioContextRef.current) === null || _a === void 0 ? void 0 : _a.state);
            });
        }
    };
    ...
    const playAudio = async (messageId, text) => {
        ...
        if (!audioContextRef.current) {
            try {
                ...
                audioContextRef.current = new (window.AudioContext || window.webkitAudioContext)(options);
            }
            catch (error) {
     ...
     * @param text Text to be converted to speech.
     */
    const connectWebSocket = (messageId, text) => {
        if (webSocketRef.current) {
            webSocketRef.current.close();
        }
        ...
        webSocketRef.current.onopen = () => {
            ...
            setIsConnected(true);
            // Send TTS request
            (_a = webSocketRef.current) === null || _a === void 0 ? void 0 : _a.send(JSON.stringify({
                type: 'tts_request',
                text: text.trim(),
                id: messageId,
            }));
     ...
     */
    const processAudioQueue = (is_end = false) => {
...
//# sourceMappingURL=audio_stream.jsx.map