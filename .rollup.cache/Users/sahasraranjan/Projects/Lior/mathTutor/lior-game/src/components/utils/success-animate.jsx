...
const SuccessAnimation = () => {
    ...
    useEffect(() => {
        const generateNewEmoji = () => {
            const newEmoji = Object.assign({ id: Date.now(), emoji: getRandomEmoji() }, getRandomPosition());
    ...
    return (<div className="fixed inset-0 pointer-events-none z-50 overflow-hidden">
      <AnimatePresence>
        {emojis.map((emojiObj) => (<motion.div key={emojiObj.id} initial={{
                y: -100,
                x: emojiObj.x,
                opacity: 1,
            }} animate={{
                ...
                ease: "easeInOut"
            }} className="absolute select-none text-4xl" style={{
                left: emojiObj.x,
            }}>
            {emojiObj.emoji}
          </motion.div>))}
...
//# sourceMappingURL=success-animate.jsx.map