...
export default function Screen1({ sendAdminMessage, onProceed }: FractionSubtractionProps) {
  ...
  const handleReflectionAnswer = (question: 'first' | 'second', answerValue: string) => {
    ...
    setGameStateRef(prev => ({
      ...
      screen1State: {
        ...prev.screen1State,
        [question === 'first' ? 'firstAnswer' : 'secondAnswer']: answerValue
      }
    }));
  }
            ...
            <div className="flex items-center gap-4">
              <div className="flex -space-x-[3px]">
                {[...Array(fraction1.denominator)].map((_, index) => (
                  <div
                    key={index}
                    draggable={currentStep === 2 && index < selectedPieces && !isPieceDropped(index)}
                    onDragStart={(e) => handleDragStart(e, index)}
                    className={`w-16 h-16 relative cursor-pointer border-[3px] border-[#906547] ${index < selectedPieces && !isPieceDropped(index)
                      ? 'bg-gradient-to-br from-[#5B361B] to-[#432611]'
                      : 'bg-gradient-to-br from-[#906547] to-[#785339]'
                      } ${currentStep === 2 && index < selectedPieces && !isPieceDropped(index) ? 'cursor-grab' : ''}`}
                ...
                <div className="flex flex-col items-center justify-center p-2">
                  <div className="text-2xl font-bold">{selectedPieces}</div>
